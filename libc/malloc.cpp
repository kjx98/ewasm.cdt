...
void *sbrk(size_t num_bytes) {
  ...
  // needed
  const size_t num_desired_pages =
      (sbrk_bytes + num_bytes + NBBP - 1) >> NBPPL2;
...
class memory_manager // NOTE: Should never allocate another instance of
                     // memory_manager
{
...
private:
  ...
  memory *next_active_heap() {
    ...
    // grab up to the end of the current WASM memory page provided that it has
    // 1KiB remaining, otherwise
    // grow to end of next page
    size_t heap_adj;
    if (current_memory_size % wasm_page_size <= wasm_page_size - 1024)
      heap_adj = (current_memory_size + wasm_page_size) -
                 (current_memory_size % wasm_page_size) - current_memory_size;
    else
      heap_adj = (current_memory_size + wasm_page_size * 2) -
                 (current_memory_size % (wasm_page_size * 2)) -
                 current_memory_size;
  ...
  public:
    ...
    size_t is_init() const { return _heap != nullptr; }
    ...
    char *malloc(size_t size) {
      ...
      buffer_ptr new_buff(&_heap[_offset + _size_marker], size,
                          _heap + _heap_size);
    ...
    char *malloc_from_freed(size_t size) {
      // assert(_offset == _heap_size, "malloc_from_freed was designed to only
      // be called after _heap was completely allocated");
    ...
    char *realloc_in_place(char *const ptr, size_t size,
                           size_t *orig_ptr_size) {
      const char *const end_of_buffer = _heap + _heap_size;
      ...
      *orig_ptr_size = orig_buffer.size();
      ...
      if (*orig_ptr_size > size) {
        ...
        buffer_ptr excess_to_free(new_ptr, *orig_ptr_size - size,
                                  _heap + _heap_size);
    ...
    public:
      buffer_ptr(void *ptr, const char *const heap_end)
          : _ptr(static_cast<char *>(ptr)),
            _size(*reinterpret_cast<size_t *>(static_cast<char *>(ptr) -
                                              _size_marker) &
                  ~_alloc_memory_mask),
            _heap_end(heap_end) {}
      ...
      void size(size_t val) {
        ...
        const size_t memory_state =
            *reinterpret_cast<size_t *>(_ptr - _size_marker) &
            _alloc_memory_mask;
      ...
      bool is_alloc() const {
        return *reinterpret_cast<const size_t *>(_ptr - _size_marker) &
               _alloc_memory_mask;
      }
    ...
    private:
      bool merge_contiguous(size_t needed_size, bool all_or_nothing) {
        ...
        while (possible_size < needed_size &&
          ...
          const size_t next_mem_flag_size =
              *reinterpret_cast<const size_t *>(_ptr + possible_size);
          // if ALLOCed then done with contiguous free memory
          if (next_mem_flag_size & _alloc_memory_mask)
            break;

          possible_size +=
              (next_mem_flag_size & ~_alloc_memory_mask) + _size_marker;
        }
        ...
        if (possible_size > needed_size) {
          ...
          buffer_ptr freed_remainder(_ptr + needed_size + _size_marker,
...
extern "C" {
void *malloc(size_t size) { return ewasm::memory_heap.malloc(size); }

void *calloc(size_t count, size_t size) {
  void *ptr = ewasm::memory_heap.malloc(count * size);
  memset(ptr, 0, count * size);
  return ptr;
}

void *realloc(void *ptr, size_t size) {
  return ewasm::memory_heap.realloc(ptr, size);
}

void free(void *ptr) { return ewasm::memory_heap.free(ptr); }
}
