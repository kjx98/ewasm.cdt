...
__attribute__((noinline)) void *memcpy(void *__restrict destination,
...
__attribute__((noinline)) void *memset(void *__restrict in, int c, size_t len) {
  uint8_t *in_ptr = (uint8_t *)in;
  c &= 0xff;
  if (len > 8) {
    // set in 8-byte chunks
    uint64_t c64 = c + (c << 8) + (c << 16) + (c << 24) +
                   (((uint64_t)c) << 32) + (((uint64_t)c) << 40) +
                   (((uint64_t)c) << 56);
    while (len > 7) {
      *(uint64_t *)in_ptr = c64;
      in_ptr += 8;
